...
/**
 * Mirror Sent.dm delivery callbacks onto campaign_messages when external_id matches.
 */
export async function reconcileCampaignMessageDeliveryByExternalId(
  supabase: SupabaseClient,
  input: {
    externalIdTrimmed: string;
    deliveryStatus: string;
  ...
  if (!input.externalIdTrimmed) {
    return { campaignId: null };
  }

  const failedDelivery = ["failed", "bounced", "rejected", "complained"].includes(
    input.deliveryStatus.toLowerCase()
  );

  const patch: Record<string, unknown> = {
    delivery_status: input.deliveryStatus,
    updated_at: input.touchedAtIso,
  };
  if (failedDelivery) {
    patch.status = "failed";
    patch.failed_at = input.touchedAtIso;
    patch.error_message = `Delivery ${input.deliveryStatus} (webhook)`;
  }

  const { data, error } = await supabase
    .from("campaign_messages")
    .update(patch)
    .eq("external_id", input.externalIdTrimmed)
    .select("campaign_id")
    .maybeSingle();

  if (error) {
    if (shouldLogDeliveryWarn(`campaign_messages:${error.message.slice(0, 140)}`)) {
      console.warn("[delivery-status] campaign_messages update:", error.message);
    }
    return { campaignId: null };
  }

  const campaignId =
    data && typeof (data as { campaign_id?: unknown }).campaign_id === "string"
      ? (data as { campaign_id: string }).campaign_id
      : null;
  ...
  if (campaignId) {
    try {
      ...
      console.warn("[delivery-status] campaign rollup:", e);
    }
  }
